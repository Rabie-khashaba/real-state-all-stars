...
<div class="container ">
  <div class="row g-4">
    ...
    <div class="profile-card text-center p-3 shadow-sm rounded bg-white">
        ...
        <!-- عدد التصويتات -->
        <!-- عدد التصويتات مع خطين -->
<!-- عدد التصويتات مع خطين علوي وسفلي -->
        <div class="my-3">
        ...
        <!--    <input type="text" class="form-control form-control-sm text-center" value="{{ url('/profile/' . auth()->id()) }}"  readonly>-->
    ...
    <div class="col-md-6 middle mb-5">
           ...
           <div class="mb-3 personal-info personal-info-ltr">
                ...
                <table class="info-table" style="margin-left: 20px;">
                    ...
                    <tr>
                        ...
                        <td class="info-value">{{$contestant->user->email}}</td>
                    </tr>
                    <tr>
                        <td class="info-label">Phone:</td>
                        <td class="info-value">{{$contestant->phone}}</td>
                    </tr>
                </table>
            </div>
        ...
        <div class="row">
                <!-- Second Pair: Expertise + Destination -->
                @if($contestant->socials && $contestant->socials->count() > 0)
                    <div class="col-12">
                        <div class="info-label">Social Media</div>
                        @foreach($contestant->socials as $social)
        ...
        <!-- Video Tab -->
            <div class="tab-pane fade" id="video">
                @include('website.profile.videoSubmission')
            </div>
        </div>
        </div>
    </div>
    ...
    <!-- <div class="col-md-3 right mb-5" style="height: 466px;">
      <div class="activation-box">
        <div style="background-color:#FFECCD; color:#FF9D00;" class="badge mb-3">{{__('profile.Pending Activation')}}</div>
            <p style="font-family: 'NowR', sans-serif; font-size: 12px;">
            {{__('profile.Choose the appropriate method to activate your account')}}
            </p>
...
@elseif($contestant->status == 'rejected')
    {{-- ❌ Account Rejected --}}
    <div class="col-md-3 right mb-5" style="height: 466px;">
        <div class="activation-box p-3 border rounded">
            <div style="background-color:#FFD6D6; color:#FF4C4C;" class="badge mb-3">
                {{ $contestant->status }} 
            </div>
            <p style="font-family: 'NowR', sans-serif; font-size: 12px;">
            {{ $contestant->standout_reason ?? 'Your application has been rejected. Please contact support for more information.' }}
            </p>
        </div>
    </div>
 ...
 @else
    ...
    <div class="col-md-3 right mb-5" style="height: 466px;">
        <div class="activation-box p-3 border rounded">
             @if(isset($contestant->status) && $contestant->status === 'pending')
                <div style="background-color:#FFECCD; color:#FF9D00;" class="badge mb-3">
                    {{ $contestant->status }} Approve
                </div>
            @endif
            <div class="d-flex justify-content-between align-items-center mb-4 mt-4">
                <span class="pA">{{__('profile.Your account is now activated')}}</span>
                <button class="btn btn-sm btnssa">{{__('profile.Activated')}}</button>
            </div>

             @foreach ($contestant->contestantStageReviews as $review)
                <div class="d-flex justify-content-between align-items-center mb-4">

                    @if ($review->is_winner)
                        <span class="pA">{{__('profile.You are winner at')}}</span>
                        <button class="btn btn-sm btnssa">{{__('profile.Stage')}} {{ $review->stage_number }}</button>

                    @elseif ($review->status === 'rejected')
                        <span class="pA">{{__('profile.Good Luck')}}</span>
                        <button class="btn btn-sm btnsa">{{__('profile.Good Luck')}}</button>
