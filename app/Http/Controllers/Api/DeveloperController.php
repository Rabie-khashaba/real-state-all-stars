...
class DeveloperController extends Controller
{
    public function index()
    {
        ...
        $data = $developers->map(function($developer) {
            return [
                'id' => $developer->id,
                'logo' => $developer->logo ? config('app.image_domain') . '/storage/' . $developer->logo : null,
                'name_ar' => $developer->name_ar ?? null,
                'name_en' => $developer->name_en ?? null,
                'description_ar' => $developer->description_ar ?? null,
                'description_en' => $developer->description_en ?? null,
                'roles' => $developer->roles ?? null,
                'is_visible' => $developer->is_visible ?? null,
    ...
    public function projects($developerId)
    {
        $developer = ProjectDeveloper::find($developerId);
        if (!$developer) {
            return response()->json(['message' => 'Developer not found'], 404);
        }

        $projects = $developer->projects()->with(['country', 'city'])->get();
    ...
    public function project($developerId, $projectId)
    {
        $developer = ProjectDeveloper::find($developerId);
        if (!$developer) {
            return response()->json(['message' => 'Developer not found'], 404);
        ...
        $project = $developer->projects()->with(['country', 'city'])->find($projectId);
        if (!$project) {
